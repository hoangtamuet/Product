...
myApp.controller('productsCtrl', function ($scope, $http) {
    var getData = function () {
        $http({
            method: "GET",
            url: "http://localhost:1000/api/product"
    ...
    $scope.postData = function (id, name, price, quantity) {
        ...
        $scope.ND = parameter;
        $http.post("http://localhost:1000/api/product/post", parameter).then(function (response) {
            if (response.data) $scope.msg = "Post Data Submitted Successfully!";
            //alert("ss");
            getData();
    ...
    $scope.editData = function (id, name, price, quantity) {
        var parameter = JSON.stringify({ id: id, name: name, price: price, quantity: quantity });
        $scope.ND = parameter;
        $http.put("http://localhost:1000/api/product/put", parameter).then(function (response) {
    ...
    $scope.deleteData = function (id) {
        var url = "http://localhost:1000/api/product/delete/" + id;
